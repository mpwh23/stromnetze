...
function StairLayoutPanel({ room, rect, dragging, onBeginWindowMove, onBeginWindowResize, onConfirm, layers, onCreateLayer }: StairLayoutPanelProps) {
  ...
  const marginM = Math.max(0.25, Math.max(safeBounds.width, safeBounds.height) * 0.08)
  const viewWidth = Math.max(0.4, (safeBounds.width + marginM * 2) / zoom)
  const viewHeight = Math.max(0.4, (safeBounds.height + marginM * 2) / zoom)
  const center = centroid(room.vertices)
  const viewBox = { x: center.x - viewWidth / 2, y: center.y - viewHeight / 2, width: viewWidth, height: viewHeight }
  const handleSize = Math.max(0.045, Math.min(safeBounds.width || 1, safeBounds.height || 1) * 0.025 / zoom)
  ...
  function defaultElement(kind: StairEditorElementKind, point: PointM): StairEditorElement {
    const defaultWidth = Math.min(Math.max(0.55, safeBounds.width * 0.22), Math.max(0.18, safeBounds.width))
    const defaultHeight = kind === 'stair' ? Math.min(Math.max(1.0, safeBounds.height * 0.36), Math.max(0.18, safeBounds.height)) : Math.min(Math.max(0.55, safeBounds.height * 0.22), Math.max(0.18, safeBounds.height))
  ...
  function renderEdgeAssignment(assignment: StairFloorEdgeLinkModel) {
    ...
    const offset = clamp(Math.min(viewBox.width, viewBox.height) * 0.1, 0.22, 0.78)
    const boxWidth = clamp(viewBox.width * 0.42, 2.05, 3.55)
    const boxHeight = clamp(viewBox.height * 0.2, 1.05, 1.55)
    const popup = {
      x: clamp(mid.x + normal.x * offset - boxWidth / 2, viewBox.x + 0.02, viewBox.x + viewBox.width - boxWidth - 0.02),
      y: clamp(mid.y + normal.y * offset - boxHeight / 2, viewBox.y + 0.02, viewBox.y + viewBox.height - boxHeight - 0.02),
    }
    const label = assignment.layerName || (assignment.layerId === '__new__' ? 'Neue Ebene' : 'Ebene waehlen')
    return (
      <g key={assignment.id} className={`stair-editor-edge-assignment ${assignment.kind} ${selectedEdgeAssignmentId === assignment.id ? 'selected' : ''}`} onMouseDown={(event) => { event.stopPropagation(); setSelectedEdgeAssignmentId(assignment.id); setSelectedElementId('') }}>
        ...
        <circle cx={a.x} cy={a.y} r={handleSize * 0.78} />
        <circle cx={b.x} cy={b.y} r={handleSize * 0.78} />
        <text className="stair-editor-edge-canvas-label" x={mid.x + normal.x * (offset * 0.72)} y={mid.y + normal.y * (offset * 0.72)} textAnchor="middle">{stairEdgeKindLabel(assignment.kind)} - {label}</text>
        <foreignObject x={popup.x} y={popup.y} width={boxWidth} height={boxHeight}>
  ...
  function addElement(kind: StairEditorElementKind, point: PointM) {
    ...
    const element = defaultElement(kind, point)
    if (!elementInsideObject(element)) return
    pushStairEditorUndoSnapshot()
  ...
  function renderDirection(element: StairEditorElement) {
    ...
    const len = Math.max(0.08, Math.min(element.widthM, element.heightM) * 0.28)
    return (
      <g className="stair-editor-direction" transform={`rotate(${normalizeRightAngle(element.rotationDeg)} ${cx} ${cy})`}>
        <line x1={cx} y1={cy + len} x2={cx} y2={cy - len} />
        <path d={`M ${cx - len * 0.38} ${cy - len * 0.55} L ${cx} ${cy - len} L ${cx + len * 0.38} ${cy - len * 0.55}`} />
      </g>
    )
  }
  ...
  function renderStairStepLines(element: StairEditorElement) {
    ...
    if (element.kind === 'stair') {
      ...
      for (let index = 1; index < steps; index += 1) {
        const y = element.yM + (element.heightM * index) / steps
        lines.push(<line key={`step-${index}`} className="stair-editor-step-line" x1={element.xM} y1={y} x2={element.xM + element.widthM} y2={y} />)
      }
      return <g className="stair-editor-step-lines">{lines}</g>
    }
    if (element.kind === 'turn') {
      const origin = element.mirrored ? { x: element.xM + element.widthM, y: element.yM + element.heightM } : { x: element.xM, y: element.yM + element.heightM }
      const lines = []
      for (let index = 1; index < steps; index += 1) {
        const t = index / steps
        const target = element.mirrored ? { x: element.xM + element.widthM * (1 - t), y: element.yM } : { x: element.xM + element.widthM * t, y: element.yM }
        lines.push(<line key={`turn-step-${index}`} className="stair-editor-step-line" x1={origin.x} y1={origin.y} x2={target.x} y2={target.y} />)
      }
  ...
  return (
    <>
      ...
      <div className="canvas-toolbox stair-editor-toolbar" aria-label="Treppenwerkzeuge" onMouseDown={(event) => event.stopPropagation()}>
        <button type="button" className={tool === 'move' ? 'active' : ''} title="Verschieben" aria-label="Verschieben" data-stair-tool="move" onClick={() => setTool('move')}>↖</button>
        <button type="button" title="Drehen 90 Grad" aria-label="Drehen 90 Grad" data-stair-tool="rotate" onClick={rotateSelected} disabled={!selectedElement}>⟳</button>
        <button type="button" title="Biegung spiegeln" aria-label="Biegung spiegeln" data-stair-tool="mirror" onClick={mirrorSelected} disabled={!selectedElement || selectedElement.kind !== 'turn'}>⇄</button>
        <button type="button" title="Canvas herauszoomen" aria-label="Canvas herauszoomen" data-stair-tool="zoom-out" onClick={() => setZoom((current) => clamp(round3(current / 1.1), 0.45, 3.5))}>−</button>
        <button type="button" title="Canvas hineinzoomen" aria-label="Canvas hineinzoomen" data-stair-tool="zoom-in" onClick={() => setZoom((current) => clamp(round3(current * 1.1), 0.45, 3.5))}>+</button>
        <button type="button" className={tool === 'align' ? 'active' : ''} title="Linien ausrichten" aria-label="Linien ausrichten" data-stair-tool="align" onClick={() => setTool((current) => current === 'align' ? 'move' : 'align')}>⇔</button>
            ...
            {elements.map((element) => {
              ...
              return (
                <g key={element.id} className={`stair-editor-element ${selected ? 'selected' : ''} ${element.kind}`} onMouseDown={(event) => startElementDrag(event, element)}>
                  <rect x={element.xM} y={element.yM} width={element.widthM} height={element.heightM} rx={Math.min(element.widthM, element.heightM) * 0.04} />
...
function BlueprintWallObjectConnections({ wallObjects, rooms }: { wallObjects: ApiWallObject[]; rooms: Room[] }) {
  const byGroup = wallObjects
    .filter((item) => item.objectType === 'door' && item.groupId)
  ...
  const connectors = Object.entries(byGroup).flatMap(([groupId, groupItems]) => {
    const segments = groupItems
      ...
      .filter((segment): segment is { left: PointM; right: PointM } => Boolean(segment))
    if (segments.length < 2) return []
    const pair = bestBlueprintConnectionPair(segments)
    return [{ groupId, points: [pair.a.left, pair.a.right, pair.b.right, pair.b.left] }]
  })
...
function blueprintWallObjectSegment(item: ApiWallObject, rooms: Room[]): { left: PointM; right: PointM } | null {
  ...
  const half = clamp(item.w / 2, 0.005, 0.48)
  ...
  return { left: pointAt(leftT), right: pointAt(rightT) }
}

function bestBlueprintConnectionPair(segments: { left: PointM; right: PointM }[]) {
...
type RenderedProjectedRoom = { room: Room; outerWall: boolean; selectedRoom: boolean; projected: ProjectedRoom3D }
...
function WallDetailEditor({
  ...
  const activeLayer = layers.find((layer) => layer.id === activeLayerId) ?? layers[0] ?? defaultFloorplanLayers()[0]
  const wallDetailRooms = useMemo(() => rooms.filter((room) => room.roomType !== 'stair' && !isStairRoom(room)), [rooms])
  ...
  const selectedWallTitle = selectedWallRoom && selectedWall
    ...
    : ''
  const selectedWallKey = selectedWall ? wallObjectKey(floor.id, selectedWall.roomId, selectedWall.edgeIndex) : ''
  ...
  async function addWallObject(type: WallLibraryItemType, x: number, y: number) {
    ...
    const optimisticByKey = targets.map((target) => {
      const item: WallPlacedObject = {
        id: `temp-${target.key}-${groupId}`,
        type,
        groupId,
        x: clamp(round3(target.reversed ? 1 - x : x), size.w / 2, 1 - size.w / 2),
        y: clamp(round3(y), size.h / 2, 1 - size.h / 2),
        w: size.w,
        h: size.h,
      }
      return { target, item }
  ...
  function updateWallObject(id: string, patch: Partial<WallPlacedObject>) {
    ...
    const affected = source.item.groupId
      ...
      : [source]
    const selectedEdge = edgePoints(selectedWallRoom.vertices, selectedWall.edgeIndex)
    setWallObjectsByKey((current) => {
      const next = { ...current }
      affected.forEach((entry) => {
        const targetRoom = rooms.find((room) => room.id === entry.roomId)
        const targetEdge = targetRoom ? edgePoints(targetRoom.vertices, entry.edgeIndex) : null
        const pair = selectedEdge && targetEdge ? wallEdgePairInfo(selectedEdge, targetEdge) : null
        const targetPatch = { ...patch }
        if (typeof patch.x === 'number' && pair?.reversed) targetPatch.x = round3(1 - patch.x)
        next[entry.key] = (next[entry.key] ?? []).map((item) => item.id === entry.item.id ? { ...item, ...targetPatch } : item)
      })
      return next
    })
    affected.forEach((entry) => {
      ...
      const targetPatch = { ...patch }
      if (typeof patch.x === 'number' && pair?.reversed) targetPatch.x = round3(1 - patch.x)
      void updateWallObjectApi(entry.item.id, targetPatch).catch((err) => console.warn('Wandobjekt konnte nicht aktualisiert werden', err))
    })
  }
  ...
  const visibleWallFacePieces = useMemo(() => projectedWallFaces.flatMap((face) => {
    if (introPhase === 'animating') return [{ face, polygon: face.points, pieceIndex: 0, opened: false }]
    const openings = wallObjectsForFace(face, floor.id, rooms, wallObjectsByKey).filter((item) => item.type === 'door' || item.type === 'window')
    return wallFaceVisiblePolygons(face.points, openings).map((polygon, pieceIndex) => ({ face, polygon, pieceIndex, opened: openings.length > 0 }))
        ...
        <div className="canvas-scroll" ref={canvasScrollRef} onWheelCapture={handleCanvasWheel} onScroll={handleCanvasScroll}>
          ...
          >
            <g className="wall-detail-stage">
              ...
              {visibleWallFacePieces.map(({ face, polygon, pieceIndex, opened }) => {
                ...
                return (
                  <polygon
                    ...
                    className={`wall-detail-face wall-detail-face-3d ${face.outerWall ? 'outer-wall-face-3d' : ''} ${face.selected ? 'selected' : ''} ${pairedHover ? 'pair-hover' : ''} ${opened ? 'opened' : ''}`}
  ...
  return (
    ...
    >
      ...
      >
        ...
        {placements.map((item) => <WallFrontPlacedObject key={item.id} item={item} placements={placements} lengthM={lengthM} heightM={heightM} wallX={x} wallY={y} wallW={drawW} wallH={drawH} selected={item.id === selectedObjectId} onBeginDrag={beginObjectDrag} />)}
      </svg>
      <div className="wall-front-meta">
        <span>Breite {formatMm(lengthM)}</span>
        <span>Hoehe {formatMm(heightM)}</span>
        <span>{placements.length} Objekt(e)</span>
...
function buildWallModel3D(floorId: string, rooms: Room[], heightM: number, wallObjectsByKey: Record<string, WallPlacedObject[]>): WallModel3D {
  ...
  const modelSourceRooms = rooms.filter((room) => room.roomType !== 'stair' && !isStairRoom(room))
...
function projectWallModelRooms(model: WallModel3D, basis: CameraBasis, selectedRoomId: string): RenderedProjectedRoom[] {
  return model.rooms.map((modelRoom) => {
...
function wallObjectsForFace(face: RenderedWallFace, floorId: string, rooms: Room[], wallObjectsByKey: Record<string, WallPlacedObject[]>) {
  ...
  rooms.forEach((room) => {
    room.vertices.forEach((_, edgeIndex) => {
      ...
      const reversed = distance(faceEdge.a, otherEdge.b) + distance(faceEdge.b, otherEdge.a) <= distance(faceEdge.a, otherEdge.a) + distance(faceEdge.b, otherEdge.b)
      objects.forEach((item) => {
        if (item.groupId && ownGroupIds.has(item.groupId)) return
        mirrored.push({ ...item, id: `${item.id}-mirror-${room.id}-${edgeIndex}`, x: reversed ? 1 - item.x : item.x })
      })
    })
  })
...
function findOpeningPartnerFace(
  ...
  const candidates = faces.flatMap((candidate) => {
    ...
    const candidateEdge = edgePoints(candidate.room.vertices, candidate.edgeIndex)
    if (!candidateEdge) return []
    const pair = wallEdgePairInfo(faceEdge, candidateEdge)
    if (!pair?.paired) return []
    const candidateKey = wallObjectKey(floorId, candidate.room.id, candidate.edgeIndex)
    const candidateOwn = (wallObjectsByKey[candidateKey] ?? []).find((candidateItem) => candidateItem.groupId === item.groupId)
    const candidateItem = candidateOwn ?? { ...item, x: pair.reversed ? 1 - item.x : item.x }
    const centerDistance = openingCenterDistance(face.points, item, candidate.points, candidateItem)
    return [{ face: candidate, item: candidateItem, score: centerDistance }]
  })
