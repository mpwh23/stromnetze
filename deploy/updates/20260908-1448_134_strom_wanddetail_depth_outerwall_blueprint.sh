...
SCRIPT_NAME="20260908-1448_134_strom_wanddetail_depth_outerwall_blueprint"
...
COMMIT_MESSAGE="Update 134: Treppenhaus-Tiefenebenen und Aussenwand-Tueren/Blueprint"
...
if [ "$(uname -s)" = "Darwin" ] && [ -z "$MODE" ]; then
  ...
  echo ""
  echo "1) Aktuellen Stage-Source + Update 134 nach GitHub pushen ..."
...
if [ "$MODE" = "--git-phase-root" ]; then
  ...
  echo "1.3) Update 134 auf Git-Source anwenden und Build pruefen ..."
  PATCH_ONLY=1 \
    STAGE_DIR="$GIT_TMP" \
    PROD_DIR="$STAGE_DIR" \
    BACKUP_BASE=/tmp \
...
log "=== $SCRIPT_NAME / Pi ==="
log "Ziel: robuste Treppenhaus-Tiefenebenen sowie korrekte Aussenwand-Tueren und Blueprint-Darstellung."
log "Stage: $STAGE_DIR"
...
log ""
log "Update 134 wurde erfolgreich in Stage gebaut und deployed. Produktion wurde nicht geaendert."
log ""
