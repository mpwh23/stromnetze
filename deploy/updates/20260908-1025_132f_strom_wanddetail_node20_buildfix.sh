...
SCRIPT_NAME="20260908-1025_132f_strom_wanddetail_node20_buildfix"
...
COMMIT_MESSAGE="Update 132f: Wanddetail 3D/Popup/Blueprint plus robuster Node-20-Build"
...
if [ "$(uname -s)" = "Darwin" ] && [ -z "$MODE" ]; then
  ...
  echo ""
  echo "1) Aktuellen Stage-Source + Update 132f nach GitHub pushen ..."
...
if [ "$MODE" = "--git-phase-root" ]; then
  ...
  GIT_TMP="/tmp/${SCRIPT_NAME}_gitrepo"
  GIT_BUILD_BACKUP="/tmp/${SCRIPT_NAME}_gitbackup"
  ...
  echo "1.3) Update 132f auf Git-Source anwenden und Build pruefen ..."
  PATCH_ONLY=1 \
    STAGE_DIR="$GIT_TMP" \
    PROD_DIR="$STAGE_DIR" \
    BACKUP_BASE=/tmp \
...
log ""
log "Update 132f wurde erfolgreich in Stage gebaut und deployed. Produktion wurde nicht geaendert."
log ""
